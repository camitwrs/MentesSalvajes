...
function IlustradorPage() {
  ...
  const fetchIlustraciones = () => {
    setLoading(true); // Activar el spinner antes de hacer la solicitud
    getAllIlustracionesRequest()
      ...
      .finally(() => {
        setLoading(false); // Desactivar el spinner cuando se recibe la respuesta
      });
  };
  ...
  const pendientes = ilustraciones.filter(
    (item) => item.estadoilustracion.toLowerCase() === "pendiente"
  ).length;

  const completadas = ilustraciones.filter(
    (item) => item.estadoilustracion.toLowerCase() === "completado"
  ).length;
