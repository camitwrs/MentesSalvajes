...
import { getTotalAlternativasRespondidasRequest, getAlternativasPorPreguntaRequest } from "../../api/alternativas"
...
const agruparRespuestasPorPregunta = (respuestas) => {
  ...
  return Object.entries(agrupadas).map(([idpregunta, alternativasMap]) => ({
    ...
    alternativas: Object.entries(alternativasMap).map(([idalternativa, total]) => ({
...
const ResumenPage = () => {
  ...
  const renderChart = (pregunta) => {
    ...
    const alternativasCompletasMap = Object.entries(alternativasTexto).reduce((acc, [id, texto]) => {
    ...
    switch (tipo) {
      case "pie":
        return <SimpleChart labels={labels} series={values} title="Respuestas" windowWidth={windowWidth} />
      case "column":
        return (
          <ColumnChart series={[{ name: "Respuestas", data: values }]} categories={labels} windowWidth={windowWidth} />
        )
      case "radar":
        return (
          <RadarChart series={[{ name: "Respuestas", data: values }]} categories={labels} windowWidth={windowWidth} />
        )
      case "angle":
        return <AngleChart series={values} labels={labels} windowWidth={windowWidth} />
      default:
        return null
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 py-6 sm:py-8 md:py-10 px-3 sm:px-4 md:px-6 lg:px-8">
      <div className="max-w-screen-lg mx-auto">
        <div className="mb-6 sm:mb-8">
          ...
          <h1
            className={`${windowWidth < 480 ? "text-xl" : windowWidth < 768 ? "text-2xl" : "text-3xl"} font-extrabold text-gray-900 mt-6 mb-8 text-center`}
          >
            ...
            {preguntas
              ...
              .map((pregunta) => (
                ...
                >
                  <div>
                    <h2
                      className={`${windowWidth < 480 ? "text-sm" : windowWidth < 768 ? "text-base" : "text-lg"} font-semibold text-gray-800`}
                    >
                      {pregunta.textopregunta}
                    </h2>
                    <p className={`${windowWidth < 768 ? "text-xs" : "text-sm"} text-gray-500 mt-1`}>
                      Total de respuestas: {getTotalRespuestasPregunta(pregunta.idpregunta)}
                    </p>
                  </div>

                  <div className="w-full overflow-x-auto">
                    <div className={`${windowWidth < 480 ? "min-w-[280px]" : "min-w-full"}`}>
